...
/* 文件名称 */
static char YmodemName[YMODEM_FILE_LEN];
/* Ymodem指令 */
#define SOH 0x01
#define STX 0x02
#define EOT 0x04
	...
	while(YmodemTime--)
	{
		switch(ShellUart.RxFlag)
		{
			...
			case YmodemOk:
				...
				while(RepeatTime--)
				{
					/* 133/135字节在115200波特率大约需要12ms传输完毕 */
					ShellDelay(25);
					if(ShellUart.RxFlag == YmodemOk)
					{
						/* 可能没有接到帧 */
						ShellUart.RxLen = 0;
						ShellUart.StartRx(ShellUart.UartHandle, ShellUart.RxData, ShellConsoleMaxSize);
			...
			case YmodemWait:
				...
				if((YmodemTime % 25) == 0)//如果Ymodem-1K超时失败可能是这个地方等待短了可以把25改大一点
				{
					/* 可能没有接到在发送一次 */
					ShellUart.RxFlag = YmodemRepeat;
				}
...
/* 检查Ymodem帧完整 */
void YmodemCheak(uint16_t Size)
{
	switch(ShellUart.RxFlag)
	{
		case YmodemWait:
		...
		case YmodemEOT:
			/* 确认EOT */
			if((Size == 1) & (ShellUart.RxData[0] == EOT))
			{
		...
		case YmodemOk:
			if(ShellUart.RxData[0] == SOH)
			{
				ShellUart.RxLen += Size;
				switch(ShellUart.RxLen)
				{
					case 133:
						/* 得到了下一个文件的起始帧 */
						ShellUart.RxFlag = YmodemGet;
